...
All data was gathered from the project TCGA-COAD from the GDC data portal: https://portal.gdc.cancer.gov/projects/TCGA-COAD

The two files *analyte.tsv* and *clinical.tsv* were retrieved from the "Biospecimen" archive, and further used to retrieve the read count datasets from the online data portal.
...
```{r}
id_cancer_patients_normal <- get_sample_ids(RNA_samples, sample_type = "normal", num_samples = 20)
id_cancer_patients_cancer <- get_sample_ids(RNA_samples, sample_type = "cancer", num_samples = 20)
...
metadata_id_patients <- data.frame(ID = id_cancer_patients, Status = labels)
```
