...
```{r}
top_genes <- topgenes$table$genes
```
...
```{r}
# doesn't work ???
left_join(miRNA_log_long, metadata, by = "TCGA_ID")
```

```{r}
# still need to work on the cancer / normal data :( Can't figure out at all how to add this information from metadata.. 
miRNA_log_long |>
