...
```{r}
miRNA_log <- readRDS(paste0(own_path, "/group_04_project/TCGA_data/miRNA_aug.RDS"))
miRNA_metadata <- readRDS(paste0(own_path, "/group_04_project/TCGA_data/miRNA_metadata_aug.RDS"))
mRNA_log <- readRDS(paste0(own_path, "/group_04_project/TCGA_data/miRNA_aug.RDS"))
...
plot1 <- ggplot(miRNA_metadata, aes(x = gender, y = age_at_index, fill = gender)) + geom_boxplot() +
...
plot2 <- ggplot(mRNA_metadata, aes(x = gender, y = age_at_index, fill = gender)) + geom_boxplot() +
  labs(x = "", y = "", title = "mRNA") +
  facet_wrap(~ "Gender", ncol = 2, scales = "free_x")
...
ggplot(cancer_stage_df, aes(x = stage, fill = stage)) +
  geom_bar() +
