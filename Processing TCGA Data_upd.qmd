...
```{r}
# labels <- c(rep("normal", length(id_cancer_patients_normal)), rep("cancer", length(id_cancer_patients_cancer)))
# data_frame <- data.frame(ID = id_cancer_patients, Label = labels)
# 
# metadata <- tibble(
...
Some of the samples that were requested by using the lists "id_cancer_patients" do not have expression data. The metadata that was composed from two lists should therefore be filtered to include the IDs that contain information about the expression.

```{r}
list <- colnames(expression_data)

metadata_id_patients <- metadata_id_patients |>
  filter(ID %in% list)
```

```{r}
# will be subsetted from the miRNA_metadata + miRNA_data
metadata_id_patients
```
...
```{r}
# making the TCGA IDs column name in metadata about the tissue type matching the miRNA_log_long
colnames(metadata_id_patients)[colnames(metadata_id_patients) == "ID"] <- "TCGA_ID"
miRNA_log_long <- merge(miRNA_log_long, metadata_id_patients, by="TCGA_ID")
```

```{r}
# merging the miRNA data with the clinical metadata about the patients
colnames(miRNA_metadata)[colnames(miRNA_metadata) == "case_submitter_id"] <- "TCGA_ID"

miRNA_log_long <- miRNA_log_long |>
  mutate(TCGA_ID = str_sub(TCGA_ID, nchar(TCGA_ID) - 20, nchar(TCGA_ID) - 8))

miRNA_log_long <- merge(miRNA_log_long, miRNA_metadata, by="TCGA_ID")
```

```{r}
# run
saveRDS(miRNA_log_long, file = paste0(own_path, "/group_04_project/data/miRNA_log_long.RDs"))
```
